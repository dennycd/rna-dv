...
extern float T;    //threshold for a stem being good.
extern int TRACE; 
...
/*main()
{
  char Seq[31] = "CGAGGGGCGGUUGGCCUCGUAAAAAGCCGC";
  Seq[30] = '\0';
  GenerateStemList(30, Seq);
  InitHotspots(10, 35); 
  return;
}*/
...
void GenerateStemList(int length, char* sequence, char* constraint)
{
  ...
  for (i = 1; i < length; i++) {
	...
	else delta = -sPair(length-1, sequence, i, jo);  
     ...
     //if record = 1, marking the trace, and also record the stem into hotspot list.
{
    ...
    if (record == 1) {
      ...
      if (hl > 2 && V[si][sj] >= T)   //at least two base pairs
