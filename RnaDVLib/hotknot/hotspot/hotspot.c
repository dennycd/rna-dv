...
int secondaryStruct(char *sequence, int length, struct Node *currentNode, 
                    struct Node *rootNode, int MaxHotspots);
...
/*char* structure points to the output of fold.c, 
  ...
  function returns the number of hotspots if it finds substructures with energy lower than T, 
  T is a global variable, which is threshold
...

{
  ...
  while (i <= len) {
	    ...
	    ee = ee;
	    if (TRACE == 1) printf("substructure %d-%d, %d-%d with energy %d  T  %f\n", i,p,q, j, ee, T);
...
int secondaryStruct(char *sequence, int length, struct Node *currentNode, struct Node *rootNode, int MaxHotspots)
{
  ...
  temp_constraint = (char*)malloc(sizeof(char)*(length+1));
   ...
   currentNode->score= score(length,sequence,currentNode->secStructure,TRACE);
	...
	for(i=0;i<length;i++)
	  {
	   ...
	   min_en = currentNode->score;
	     ...
	     energy = secondaryStruct(sequence, length, currentNode->children[i], rootNode, MaxHotspots);
